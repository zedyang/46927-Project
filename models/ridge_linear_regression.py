...
class RidgeLinearRegression2Blocks(EmpiricalRiskOptimizer):
    """
    Only for debug purposes.
    """

    def __init__(self, **kwargs):
        super(RidgeLinearRegression2Blocks, self).__init__(**kwargs)
        self.p1 = None
        self.C = kwargs.pop('C')
        self.hyperparams = ['C']
    ...
    def get_emp_risk(self):
        ...
        #ridge loss
        ridge_loss = tf.add(
            tf.reduce_sum(tf.multiply(b1, b1), name = 'block1'),
            tf.reduce_sum(tf.multiply(b2, b2), name = 'block2'), 
            name='ridge_loss'
        )

        # must return individual losses and total empirical risk
        losses = tf.add(tf.square(self.y_tr - y_hat), 
                        tf.multiply(self.C, ridge_loss), name = 'ridge_total_loss')
